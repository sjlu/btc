...
var getTradesPaginated = function(since, before, cb) {
  ...
  makeRequest('GET', '/products/' + product + '/trades', req, function(err, data, headers) {
    winston.verbose('making request', req);
    if (err) return cb(err);
    cb(null, {
      data: data,
...
module.exports.getTrades = function getTrades(since, cb) {
  ...
  getTradesPaginated(since, null, function(err, data) {
    ...
    if (data.nextPage <= since) {
      ...
      async.eachLimit(_.range(-1*data.nextPage, -1*since, 100), 2, function(before, cb) {
...
module.exports.getBestBids = function(cb) {

  makeRequest('GET', '/products/' + product + '/book', {}, function(err, data) {
    if (err) return cb(err);

    return cb(null, {
      buy: parseFloat(data.bids[0][0]),
      sell: parseFloat(data.asks[0][0])
    });
  });
